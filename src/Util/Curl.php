...
class Curl
{
	...
	/**
	 * Creates a new cURL session with the following default options applied.
	 * Use {@link #SetOpt} to change them. Proxy and SSL options should be set
	 * in the settings.ini file since they are more commonly configured options.
	 *
	...
	public function CreateSession($url)
	{
		...
		if (defined('HTTP_PROXY_USER') && defined('HTTP_PROXY_PASSWORD') && HTTP_PROXY_USER != '' && HTTP_PROXY_PASSWORD != '') {
			$this->SetOpt($ch, CURLOPT_PROXYUSERPWD, HTTP_PROXY_USER . ':' . HTTP_PROXY_PASSWORD);
		}
