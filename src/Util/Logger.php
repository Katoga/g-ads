...
/**
 * A logger class which registers appenders (any writable handle location which can be fopen'ed)
 * with a unique name for that log.
 * The library will write all incoming/outgoing SOAP XML
 * to <var>SOAP_XML_LOG</var> and all records of requests to <var>REQUEST_INFO_LOG</var>.
 *
...
class Logger
{
	...
	/**
	 * Logs the given log to a stream represented by <var>$phpIoStream</var>.
	 * The availble streams are listed here:
	 *
	...
	/**
	 * Sets the logging level for a given log.
	 * Only messages at that log level or higher will be written to the log.
	 *
	 * @param string $log the unique name of the log
	...
	{
		if (array_key_exists($log, self::$logToMap)) {
			$logLevel = array_key_exists($log, self::$logLevelMap) ? self::$logLevelMap[$log] : self::$INFO;
	...
	/**
	 * Determines if a message of a given level should be written to a log of a given level.
	 *
	...
	private static function ShouldLog($logLevel, $messageLevel)
	{
		...
		return array_search($logLevel, $levelPriority) <= array_search($messageLevel, $levelPriority);
	}

	/**
	 * Writes the message to all appenders (any writable handle location that can be fopen'ed)
	 * at a specific level.
	 *
	...
	/**
	 * Writes a message with the prepended time and level to the handle location.
	 * The handle will be opened, written to and then closed.
	 *
	 * @param string $handleLocation the location of the handle that will be fopen'ed
	 * @param string $message the message to write
	...
	private static function WriteMessage($handleLocation, $message, $level)
	{
		...
		$line = '[' . date(self::$timeFormat, time()) . ' - ' . $level . '] ' . $message . "\n";
		fwrite($fp, $line);
		fclose($fp);
	}
}
