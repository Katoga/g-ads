...
class SoapRequestXmlFixer
{
   ...
   *
   * @param boolean $addXsiTypes <var>true</var> if xsi:types should be added to all complex type elements
   * @param boolean $removeEmptyElements <var>true</var> if all empty elements should be removed from the XML request
   * @param boolean $replaceReferences <var>true</var> if element references should be replaced with a copy of the element.
   */
  public function __construct($addXsiTypes, $removeEmptyElements, $replaceReferences)
  {
    ...
    if ($object instanceof SoapHeader) {
      ...
      if (is_object($object)) {
        foreach (get_object_vars($object) as $varName => $varValue) {
          $nodeList = $xpath->query("*[local-name() = '" . $varName . "']", $node);
  ...
  private function AddXsiType(DOMNode $domNode, $object)
  {
    ...
    if (
      method_exists($object, 'getXsiTypeName')
  ...
  {
    $requestHeaderDom = $xpath->query("//*[local-name()='Envelope']/*[local-name()='Header']/*[local-name()='RequestHeader']")->item(0);
