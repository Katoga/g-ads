...
abstract class AdsUser
{
	...
	/**
	 * Gets the authenticaiton value for the <var>$authVar</var> supplied.
	 * If the <var>$authVar</var> is set, it is is used. Otherwise, the supplied
	 ...
	 * @param array $authIni the array of authentication variables from an INI file
	 ...
	 * HTTP headers and SOAP XML are logged to the soap_xml.log file and all request information
	 * is logged to the request_info.log file under the logs directory.
	 */
	...
	/**
	 * Configures the library to log basic information about all requests and the full SOAP XML
	 * request and response only when an error occurs.
	 */
	...
	/**
	 * Loads the settings for this client library.
	 * If the settings INI file located at <var>$settingsIniPath</var> cannot be loaded,
	 * then the parameters passed into this method are used.
	 *
	 * @param string $settingsIniPath the path to the settings INI file
	 * @param string $defaultVersion the default version if the settings INI file cannot be loaded
	 * @param string $defaultServer the default server if the settings INI file cannot be loaded
	 * @param string $defaultLogsDir the default logs directory if the settings INI file cannot be loaded
	 * @param string $logsRelativePathBase the relative path base for the logs directory
	 */
	 ...
	 * If it is defined but the value is different then attempt to redefine it, which will fail
	...
	/**
	 * Gets common PHP user agent parts for ads client libraries such as PHP version,
	 * operating system, browser, or if compression is being used or not.
	 *
	 * @return array An array of arrays with each inner array representing a user agent parts, e.g.: ['PHP', '5.3.2'] or ['PHP', '5.4.0'].
	 */
	 ...
	 * in theirconstructor.
	 *
	 * @param $applicationName The application name that will appear in this header.
	 */
