 ...
 * making a request as well as perform any book-keeping on the response.
 *
...
abstract class AdsSoapClient extends \SoapClient
{
	...
	/**
	 * The last SOAP XML DOMDocument request made to the server after PrepareRequest() and RemoveSensitiveInfo() have been called on it.
	 *
	 * @var DOMDocument the last SOAP XML request made to the server. Can be <var>null</var> if the last request was not proper XML
	 * @see PrepareRequest()
	 * @see RemoveSensitiveInfo()
	 */
	 ...
	 *
	 * @var DOMDocument the last SOAP XML response from the server. Can be <var>null</var> if the last request was not proper XML
	 */
	private $lastResponseDom;

	/**
	 * The last SOAP fault generated from the server.
	 * <var>null</var> if none.
	 *
	 ...
	 */
	protected function __construct($wsdl, array $options, AdsUser $user, $serviceName, $serviceNamespace)
	{
		$this->user = $user;
	...
	{
		$this->lastRequest = $this->PrepareRequest($request, $this->lastArguments, $this->lastHeaders);

		if (!empty($this->transportLayer)) {
			$response = $this->transportLayer->__doRequest($this->lastRequest, $location, $action, $version);

			return $response;
		}

		return parent::__doRequest($this->lastRequest, $location, $action, $version);
	}
	...
	/**
	 * Logs the SOAP XML to the Logger::$SOAP_XML_LOG log after the request has transformed
	 * by PrepareRequest() and both the request and response have been sanitized
	 * by RemoveSensitiveInfo().
	 *
	...
	/**
	 * Logs the request info to the Logger::$REQUEST_INFO_LOG log the request has transformed
	 * by PrepareRequest() and both the request has been sanitized by RemoveSensitiveInfo().
	 *
	 * @param string $level the log level to use
	 * @see PrepareRequest()
	 * @see RemoveSensitiveInfo()
	 ...
	 *
	 * @return s boolean <var>true</var> if there was a SOAP fault during the last call
	 */
	 ...
	 * @return string the value of the request header element or <var>null</var> if not found
	 */
	...
	/**
	 * Generates the HTTP headers for the client.
	 * By default includes OAuth2
	...
	/**
	 * Removes any sensitive information from the request XML.
	 * This method is called after the request has been made and before logging any XML.
	 *
	 * @param string $request the request just made to the server
	 * @return string the request with any sensitive information removed ready to be logged.
	 */
	protected abstract function RemoveSensitiveInfo($request);

	/**
	 * Generates the request information using the request and response.
	 * This method is called after the request has been made and RemoveSensitiveInfo()
	 ...
	 * When pseudo-namespace support is enabled class names can become very long, and this function
	 * provides an alternative way to create objects that is more readable.
	 *
