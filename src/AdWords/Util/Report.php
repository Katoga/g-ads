...
class Report
{
	...
	/**
	 * The length of the snippet to read from the request to determine if there was an error.
	 */
	...
	/**
	 * Downloads a new instance of an existing report definition.
	 * If the path parameter is specified it will be downloaded to the file at that path, otherwise
	 * it will be downloaded to memory and be returned as a string.
	 *
	 * @param mixed $reportDefinition the ReportDefinition to download or the id of a stored report definition
	...
	{
		/*
		 * This method should not be static and instantiation of this class should be allowed so we can
		 * "inject" CurlUtils, but would break too many things that rely on this method being static.
		 */
	...
	/**
	 * Tries to parse the error response xml from the AdWords API server as an object.
	 * This method is used in parsing all error responses when API version >= v201209, and in other
	 * versions when apiMode header is mentioned in the request headers.
	 *
	 * @param String $responseXml the error response xml
	 * @return Object the parsed error object, or null if the response cannot be parsed.
	 */
