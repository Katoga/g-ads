...
class AdWordsSoapClient extends AdsSoapClient
{
	...
	/**
	 * Overrides the method __doRequest().
	 * When OAuth2 authentication is used the URL parameters added.
	 *
	...
	{
		return 'effectiveUser=' . $this->GetEffectiveUser() . ' service=' . $this->GetServiceName() . ' method=' . $this->GetLastMethodName() . ' operators=' . $this->GetLastOperators() . ' responseTime=' . $this->GetLastResponseTime() . ' requestId=' . $this->GetLastRequestId() . ' operations=' . $this->GetLastOperations() . ' units=' . $this->GetLastUnits() . ' server=' . $this->GetServer() . ' isFault=' . ($this->IsFault() ? 'true' : 'false') . ' faultMessage=' . $this->GetLastFaultMessage();
	}
}
